...
def 基本面評分(股票代號):
    ...
    for 日期 in 取得候選日期():
        ...
        代號索引 = 找欄位索引(欄位, ["證券代號", "股票代號"])

        row = None
        for 候選列 in rows:
            比對值 = 候選列[代號索引] if 代號索引 is not None else 候選列[0]
            if 比對值.strip() == 股票代號:
                row = 候選列
                break
        ...
        名稱索引 = 找欄位索引(欄位, ["證券名稱", "股票名稱"])
        股票名稱 = row[名稱索引].strip() if 名稱索引 is not None else None
...
def analyze_stock(股票代號):
    ...
    基本分數, 基本說明, 基本資料, 股票名稱 = 基本面評分(股票代號)
    ...
    return {
        "code": 股票代號,
        "name": 股票名稱 if 股票名稱 else f"{股票代號} 台股",
        "price": 收盤價[-1],
